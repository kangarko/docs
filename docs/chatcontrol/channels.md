...
4. **Party** — faction/clan/party members (can also have block/world range). Set the `Party` key in the channel to one of the values listed in the `Party` comment of the `Channels` section in your settings.yml, such as `towny-town` or `simpleclans-clan`. Over proxy, party messages only reach other servers when the party plugin shares its data across them (`simpleclans-clan` and `simpleclans-ally` with SimpleClans on a shared MySQL database and its `use-bungeecord` option enabled on every server); all other parties only reach players on the sender's server. See [Compatibility](./compatibility) for the SimpleClans caveat about servers that were empty when a clan changed.
